...
import com.taxi.friend.drivers.register.DriverLicenseBackActivity;
...
public class DriverService {
    ...
    public Call<ResponseWrapper<List<DriverLocation>>> getDriverMates(int driverId) throws Exception {

        DriverRepository service = RestClient.createRestClient().create(DriverRepository.class);
        Call<ResponseWrapper<List<DriverLocation>>> result = service.getDrivers(driverId, 1, -34.634713, -58.436193);
        return result;
    }
    ...
    public Call<ResponseWrapper<DriverLocation>> createDriver(TaxiDriver taxiDriver){
        DriverRepository service = RestClient.createRestClient().create(DriverRepository.class);
